 ...
 * Section 2: CSS scroll-snap carousel, counter only
 */
...
function renderReviews(product, imageBaseDir) {
  ...
  const breakdownHtml = [5, 4, 3, 2, 1].map(star => {
    ...
    return `<div class="breakdown-row">
      <span class="breakdown-label">${star}${iconStar()}</span>
      <div class="breakdown-bar-bg" role="progressbar" aria-valuenow="${count}" aria-valuemin="0" aria-valuemax="${maxBreakdown}" aria-label="${star} sao: ${count} đánh giá">
  ...
  const photos = Array.isArray(reviews.photos) ? reviews.photos : [];
  const photosHtml = photos.length
    ? `<div class="reviews-photos-title">Photos from reviews</div>
  <div class="reviews-photos-strip" aria-label="Ảnh từ đánh giá">
    ${photos.map(p => `<img class="reviews-photo-thumb" src="${esc(imgUrl(p))}" alt="Ảnh đánh giá" loading="lazy">`).join('\n    ')}
  </div>`
    : '';

  const items = Array.isArray(reviews.items) ? reviews.items : [];

  const filterTabs = `<div class="reviews-filter-tabs" role="tablist" aria-label="Lọc đánh giá">
    <button class="review-tab active" type="button" role="tab" aria-selected="true" data-filter="all">All</button>
    <button class="review-tab" type="button" role="tab" aria-selected="false" data-filter="5">5★</button>
    <button class="review-tab" type="button" role="tab" aria-selected="false" data-filter="4">4★</button>
    <button class="review-tab" type="button" role="tab" aria-selected="false" data-filter="photo">With photo</button>
  </div>`;

  const cardsHtml = items.length
    ? items.map(item => {
        const stars = goldStars(item.stars || 5);
        const initials = (item.name || '?').charAt(0).toUpperCase();
        const photoHtml = item.photo
          ? `<img class="review-photo" src="${esc(imgUrl(item.photo))}" alt="Ảnh đánh giá của ${esc(item.name)}" loading="lazy">`
          : '';
        const verifiedHtml = item.verified
          ? `<span class="review-verified">${iconCheck()} Verified purchase</span>`
          : '';
        const regionHtml = item.region
          ? `<span class="review-region">${esc(item.region)}</span>`
          : '';
        return `<div class="review-card" data-stars="${item.stars || 5}">
      ...
      <div class="review-card-stars" aria-label="${item.stars || 5} sao" role="img">${stars}</div>
      <p class="review-text">${esc(item.text)}</p>
      ${photoHtml}
      ${item.itemLabel ? `<div class="review-item-label">Item: ${esc(item.itemLabel)}</div>` : ''}
  ...
  <div class="reviews-aggregate">
    ...
    <div class="reviews-stars-count">
      <div class="reviews-stars-row">
        <span class="reviews-stars" aria-hidden="true">${goldStars(Math.round(reviews.average))}</span>
      </div>
      <div class="reviews-total">${reviews.total} global reviews</div>
    </div>
  </div>
...
function renderAbout(product, imageBaseDir) {
  ...
  <button class="about-toggle" type="button" aria-expanded="false" aria-controls="about-content">
  ...
  <div id="about-content" class="about-content" role="region" aria-labelledby="about-toggle-label">
